...
to move
  ...
  foreach [self] of patches with [distance myself < perception-range] [
    ...
    ;; FOOD
    let foodValue [penergy] of ? * (1 - ([energy] of meTurtle / max-energy)) * ( 1 - (distance meTurtle / perception-range));

    ;; CONSPECIFIC
    let conspecificList [self] of turtles-on ?;
    let conspecificValue 1;
    let groupValue 1;
    foreach conspecificList [
      ifelse [group-number] of ? != [group-number] of meTurtle [
    ...
    ;; HOME RANGE
    let homeValue (1 - ([distance [home-base] of meTurtle] of ? / 100)) * (distance [home-base] of meTurtle / 100)


    ;; MATES
    let mateValue 1;
    foreach conspecificList [
      if [breed] of ? = females and [breed] of meTurtle = males [
...
to compete
  foreach [self] of turtles-here [
    ask ? [ set energy energy - ([fighting-ability] of myself) * aggression-cost]
  ]
  foreach [self] of turtles-on neighbors [
    ask ? [ set energy energy - ([fighting-ability] of myself) * aggression-cost]
  ]
end
